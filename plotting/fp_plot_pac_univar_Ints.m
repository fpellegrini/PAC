function fp_plot_pac_univar_Ints
...
for iit= 1:100
    try
        ...
        for ip = [10 16 17]
            ...
            %false positives
            P{1}(:,:,iit,ip1) = p_standard<0.05;
            P{2}(:,:,iit,ip1) = p_orig<0.05;
            P{3}(:,:,iit,ip1) = p_anti<0.05;
            
            %seed region
            iroi{ip1}(:,iit)=iroi_phase;  
            ip1 = ip1+1;
        end        
...
%% in case some iteration is missing, remove that iteration from all conditions

for ii =1:3
    iroi{ii}(:,a(a<=size(P{3},3)))=[];
    P{ii}(:,:,a(a<=size(P{3},3)),:)=[];
...
for ip1 = 1:3
    for iit = 1:size(iroi{1},2)%loop over iterations
        for im = 1:numel(P) %loop over metrics
            u1=[];
            u2=[];
            for ir = 1:size(iroi{ip1},1) %loop over seed regions
                cnb = find(nb(:,iroi{ip1}(ir,iit))==0.5); %find neighbors of current seed region
...
for im = 1:3
    ...
    %
    outname = [DIRFIG 'Int_' mets{im} '_uni_seedamp.eps'];
    print(outname,'-depsc');
%     outname = [DIRFIG 'neighbors_seedamp_iInt' num2str(params.iInt) '_filt' params.ifilt '_' mets{im} '_iInt.png'];
%     print(outname,'-dpng');
    close all
...
for im = 1:3
    ...
    %
    outname = [DIRFIG 'Int_' mets{im} '_uni_seedphase.eps'];
    print(outname,'-depsc');
%     outname = [DIRFIG 'neighbors_seedphase_iInt' num2str(params.iInt) '_filt' params.ifilt '_' mets{im} '_Int.png'];
%     print(outname,'-dpng');
    close all
end